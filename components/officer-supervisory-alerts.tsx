      ...
      <CardContent className="space-y-3 pt-6">
        {loading ? (
            ...
            >
              <div className="flex flex-wrap items-center justify-between gap-3">
                ...
                <div className="flex items-center gap-2 text-xs text-[#8d5a13]">
                  {!alert.is_read ? <span className="rounded-full bg-[#ff9933] px-2 py-0.5 font-semibold text-slate-900">New</span> : null}
              ...
              <div className="mt-3 flex items-center justify-between text-xs text-slate-500">
                <span>{alert.complaint_id ? complaintCodeById[alert.complaint_id] || 'Mapped complaint alert' : 'General supervisory alert'}</span>
          ...
          <div className="rounded-[1.1rem] border border-dashed border-[#eed9bd] bg-white px-4 py-5 text-sm text-[#8d5a13]">
            No higher-officer reminders are active right now.
          </div>
        )}
